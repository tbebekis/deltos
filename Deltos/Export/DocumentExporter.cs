...
public class DocumentExporter
{
    ...
    static void AppendMarkdownHtml(StringBuilder Builder, string MarkdownText)
    {
        if (!string.IsNullOrWhiteSpace(MarkdownText))
            Builder.AppendLine(Markdig.Markdown.ToHtml(MarkdownText));
    }
    /// <summary>
    ...
    static void AppendPlainTextHtml(StringBuilder Builder, string Text)
    {
    ...
    {
        StringBuilder Builder = new StringBuilder();
        Builder.AppendLine(fDocument.Title);
        Builder.AppendLine();
    ...
    {
        string Title = FormatTitle(Folder.LevelTitle, Folder.OrderIndex, Folder.Title, fOptions.FolderTitle);
    ...
    {
        string Title = FormatTitle("TextFile", File.OrderIndex, File.Title, fOptions.TextFileTitle);
        if (!string.IsNullOrWhiteSpace(Title))
            Builder.AppendLine(Title);
    ...
    string BuildHtml(bool UseSecondary, bool IncludeToc, bool UseBlackHeadings)
    {
        StringBuilder Builder = new StringBuilder();
        ...
        AppendTocItem(TocBuilder, 1, AnchorId, fDocument.Title);
        Builder.AppendLine($"<h1 id=\"{AnchorId}\">{WebUtility.HtmlEncode(fDocument.Title)}</h1>");
        foreach (Folder Folder in fDocument.Folders.OrderBy(Item => Item.OrderIndex))
            AppendFolderHtml(Builder, TocBuilder, ref AnchorIndex, Folder, UseSecondary, 2);

        foreach (TextFile File in fDocument.Files.OrderBy(Item => Item.OrderIndex))
            AppendTextFileHtml(Builder, TocBuilder, ref AnchorIndex, File, UseSecondary);

        return WrapHtml(fDocument.Title, TocBuilder.ToString(), Builder.ToString(), IncludeToc, UseBlackHeadings);
    }
    /// <summary>
    /// Appends folder HTML.
    /// </summary>
    ...
    /// <param name="UseSecondary">True to use secondary text.</param>
    /// <param name="Level">The heading level.</param>
    void AppendFolderHtml(StringBuilder Builder, StringBuilder TocBuilder, ref int AnchorIndex, Folder Folder, bool UseSecondary, int Level)
    {
        string Title = FormatTitle(Folder.LevelTitle, Folder.OrderIndex, Folder.Title, fOptions.FolderTitle);
        if (!string.IsNullOrWhiteSpace(Title))
        {
            ...
            Builder.AppendLine($"<h{Level} id=\"{AnchorId}\">{WebUtility.HtmlEncode(Title)}</h{Level}>");
        }

        foreach (Folder ChildFolder in Folder.Folders.OrderBy(Item => Item.OrderIndex))
            AppendFolderHtml(Builder, TocBuilder, ref AnchorIndex, ChildFolder, UseSecondary, Math.Min(6, Level + 1));
    ...
    /// <param name="File">The text file.</param>
    /// <param name="UseSecondary">True to use secondary text.</param>
    void AppendTextFileHtml(StringBuilder Builder, StringBuilder TocBuilder, ref int AnchorIndex, TextFile File, bool UseSecondary)
    {
        string Title = FormatTitle("TextFile", File.OrderIndex, File.Title, fOptions.TextFileTitle);
        if (!string.IsNullOrWhiteSpace(Title))
        {
            string AnchorId = CreateAnchorId(AnchorIndex++);
            AppendTocItem(TocBuilder, 3, AnchorId, Title);
            Builder.AppendLine($"<h3 id=\"{AnchorId}\">{WebUtility.HtmlEncode(Title)}</h3>");
        }

        string Text = UseSecondary ? File.Text2 : File.Text;
        if (fOptions.TreatTextFilesAsPlainText)
            AppendPlainTextHtml(Builder, Text);
        else
            AppendMarkdownHtml(Builder, Text);
    }
    /// <summary>
    ...
    public string Execute()
    {
        ...
        if (fOptions.Source.HasFlag(ExportSource.Text))
        {
            ...
            foreach (ExportLanguage Language in Languages)
            {
                ...
                {
                    string OdtSourceHtml = BuildHtml(UseSecondary, false, true);
